...
# task to block indefinitely until data is read from /dev/tty (the keyboard) and
...
#
# Extra file descriptors used:
#   3. read/write - the named pipe for task communication
#   4. read - /dev/tty for reading keyboard input
#
# Animation Story Time: When I added the custom `tput` function below it sped things up
# *so much* that it stopped looking like there were animations.  To re-add
# them, we need to be able to sleep with millisecond resolution - however that
# is not POSIX.  We will do our best to support this by 1. attempting to load
# the bash sleep bultin (failing silently) and then just attempting to run sleep
# with millisecond times (also failing silently).  Either it'll work, or we'll
# just fail - so worst case the user gets no animations.
#
